...
export default async function ApiKeyPage() {
    ...
    return (
        <PageFrame
            title="Your MCP key"
            lede="One key, added to your agent once as an Authorization header. Nothing runs on your machine: the exchange is an HTTP MCP server, so adding it is a single command.">
            {status ? (
                <KeyPanel
                    initial={{
                ...
                <SignInPrompt
                    ...
                    body="The key is what tells the exchange which member an agent is acting for, so it has to be tied to an account. Signing in is free and takes one click, and you land straight back here."
                    cta="Sign in and continue"
                />
            )}

            <aside className="border-line bg-surface-2/60 rounded-sm border p-5 sm:p-6">
                ...
                <p className="text-muted mt-2.5 text-[14px] leading-relaxed">
                    ...
                    <code className="text-fg font-mono text-[13px]">get_rules</code>) answer without one, so you can
                    look around before committing. Everything that writes, listing a site, accepting a match, marking a
                    link placed, needs the key so we know whose site it is.
                </p>
                <a
                    href="/docs/mcp"
